...
def buildTrackletDatabase(database, outDir,
    ...
    if verbose is True:
        print("Building tracklet database: {}".format(database))
...
def buildTrackDatabase(database, outDir,
    ...
    if verbose is True:
        print("Building track database: {}".format(database))
...
def buildSummaryDatabase(database, outDir,
                         verbose=Config.verbose):
    """
    Build summary database with AllObjects table, AllWindows table and the 
    WindowsMembers table. 
    ...
    if verbose is True:
        print("Building summary database: {}".format(database))
    ...
    if verbose is True:
        print("\tCreating AllWindows table...")
    con.execute("""
        CREATE TABLE AllWindows (
            windowId INTEGER PRIMARY KEY,
            completeness REAL,
            contaminationTracks REAL,
            contaminationTracklets REAL,
            nightStart INTEGER,
            nightEnd INTEGER,
            numObjects INTEGER,
            numObjectsFindableAsTracklet INTEGER,
            numObjectsFindableAsTrack INTEGER,
            numObjectsFoundAsTracklet INTEGER,
            numObjectsFoundAsTrack INTEGER,
            numDiaSources INTEGER,
            numTrueTracklets INTEGER,
            numFalseTracklets INTEGER,
            numTrueCollapsedTracklets INTEGER,
            numFalseCollapsedTracklets INTEGER,
    ...
    if verbose is True:
        print("\tCreating WindowMembers table...")
        print("")
    con.execute("""
        CREATE TABLE WindowMembers (
            windowId INTEGER, 
            objectId INTEGER,
            numDiaSources INTEGER,
            findableAsTracklet BOOLEAN,
...
def readDetectionsIntoDatabase(detsFile, con,
    ...
    input detections file. See `~analyzemops.Config.detection_file_columns`. 
    ...
    specialIds : dict, optional
        ...
        integer values. [Default = `~analyzemops.Config.detection_file_special_ids`]
    readParams : dict, optional
        Dictionary of `Pandas.read_csv` keyword arguments to use when reading in the detections file.
        [Default = `~analyzemops.Config.detection_file_read_params`]
...
def readTrackletsIntoDatabase(trackletFile, con,
    ...
    if len(trackletMembers) == 0:
        if verbose is True:
            print("{} has no tracklets. Skipping.".format(trackletFile))
        return        
...
def readManyTracksIntoDatabases(tracker, verbose=Config.verbose):
    ...
    """
    tracker.trackDatabases = []
    trackIdStart = 1
    
    for i, (trackFile, trackOutFile) in enumerate(zip(tracker.tracks, tracker.trackOuts)):
        track_con, track_db = buildTrackDatabase("tracks_" + trackFile.split("/")[-1].split(".")[0] + ".db", tracker.runDir)
        tracker.trackDatabases.append(track_db)
